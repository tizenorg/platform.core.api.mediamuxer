...
/*
 * @file mediamuxer_port_gst.c
 * @brief contain the function definitions  etc of mediamuxer gst port
...
static int gst_muxer_get_data_count(MMHandleType pHandle, int *count);
static int gst_muxer_get_track_info(MMHandleType pHandle, void *info);
static int gst_muxer_set_track(MMHandleType pHandle, int track);
static int gst_muxer_get_data(MMHandleType pHandle, char *buffer);
...
static gint __gst_handle_resource_error(mxgst_handle_t* core, int code);
static gint __gst_handle_library_error(mxgst_handle_t* core, int code);
static gint __gst_handle_core_error(mxgst_handle_t* core, int code );

static mx_ret_e _gst_create_pipeline(mxgst_handle_t *gst_handle);
static mx_ret_e _gst_destroy_pipeline(mxgst_handle_t *gst_handle);

static int _gst_set_caps(MMHandleType pHandle, media_packet_h inbuf);
...
static media_port_muxer_ops def_mux_ops = {
	...
	.get_data_count = gst_muxer_get_data_count,
	.get_track_info = gst_muxer_get_track_info,
	.set_track = gst_muxer_set_track,
	.get_data = gst_muxer_get_data,
...
static int gst_muxer_write_sample(MMHandleType pHandle, int track_index, media_packet_h inbuf)
{
	...
	if (gst_handle->video_track.track_index != -1 && track_index == gst_handle->video_track.track_index) {
		MX_I("\npushing video");
	...
	} else if (gst_handle->audio_track.track_index != -1 && track_index == gst_handle->audio_track.track_index) {
		MX_I("\npushing audio");
#ifdef ASYCHRONOUS_WRITE
		...
		case GST_MESSAGE_ERROR: {
				...
				if (error) {
					/* if (error->domain == GST_STREAM_ERROR)
						ret = __gst_handle_stream_error(gst_handle, error, msg);
					else */
...
mx_ret_e _gst_create_pipeline(mxgst_handle_t *gst_handle)
{
	...
	/* Note: Replace the arguments of gst_init to pass the command line
	args to GStreamer. */
	gst_init(NULL, NULL);
	...
	if (gst_handle->muxed_format == MEDIAMUXER_CONTAINER_FORMAT_MP4) {
		...
		ret = MEDIAMUXER_ERROR_INVALID_URI;
		goto ERROR;
	}
	gst_handle->sink =
	    gst_element_factory_make("filesink", "muxer filesink");
	...
	if (gst_handle->video_track.track_index == 1) { /* video track */
		...
		gst_bin_add_many(GST_BIN(gst_handle->pipeline), gst_handle->video_appsrc, \
		                 gst_handle->videoparse, NULL);
		/* Set video caps for corresponding src elements */
		g_object_set(gst_handle->video_appsrc, "caps", gst_caps_from_string(gst_handle->video_track.caps), NULL);
#ifdef ASYCHRONOUS_WRITE
		g_signal_connect(gst_handle->video_appsrc, "need-data", G_CALLBACK(_video_start_feed), gst_handle);
		g_signal_connect(gst_handle->video_appsrc, "enough-data", G_CALLBACK(_video_stop_feed), gst_handle);
#else
		g_object_set(gst_handle->video_appsrc, "block", TRUE, NULL);
		gst_app_src_set_stream_type((GstAppSrc *)gst_handle->video_appsrc, GST_APP_STREAM_TYPE_STREAM);
	...
	if (gst_handle->audio_track.track_index == 2) { /* Audio track */
		...
		gst_bin_add_many(GST_BIN(gst_handle->pipeline), gst_handle->audio_appsrc, \
		                 gst_handle->audioparse, NULL);
		/* Set audio caps for corresponding src elements */
		g_object_set(gst_handle->audio_appsrc, "caps", gst_caps_from_string(gst_handle->audio_track.caps), NULL);
#ifdef ASYCHRONOUS_WRITE
		g_signal_connect(gst_handle->audio_appsrc, "need-data", G_CALLBACK(_audio_start_feed), gst_handle);
		g_signal_connect(gst_handle->audio_appsrc, "enough-data", G_CALLBACK(_audio_stop_feed), gst_handle);
#else
		g_object_set(gst_handle->audio_appsrc, "block", TRUE, NULL);
		gst_app_src_set_stream_type((GstAppSrc *)gst_handle->audio_appsrc, GST_APP_STREAM_TYPE_STREAM);
#endif
	}
	...
	gst_handle->bus_watch_id = gst_bus_add_watch(bus, _mx_gst_bus_call, (gpointer)gst_handle);
...
static int gst_muxer_pause(MMHandleType pHandle)
{
	...
	if (state == GST_STATE_PLAYING) {
		if (gst_element_set_state(gst_handle->pipeline, GST_STATE_PAUSED) == GST_STATE_CHANGE_FAILURE) {
...
/* This signal callback triggers when appsrc needs mux_data. Here, we add an idle handler
 * to the mainloop to start pushing data into the appsrc */
static void _audio_start_feed(GstElement *source, guint size, mxgst_handle_t *gst_handle)
{
	/* gst_handle->audio_track.NeedData = true; */
	MX_I("\nAudio Start feeding...\n");
...
static void _video_start_feed(GstElement *source, guint size, mxgst_handle_t *gst_handle)
{
	/* gst_handle->video_track.NeedData = true; */
...
static void _audio_stop_feed(GstElement *source, mxgst_handle_t *gst_handle)
{
	/* gst_handle->audio_track.NeedData = false; */
...
static void _video_stop_feed(GstElement *source, mxgst_handle_t *gst_handle)
{
	/* gst_handle->video_track.NeedData = false; */
	MX_I("\nVideo Stop feeding...\n");
