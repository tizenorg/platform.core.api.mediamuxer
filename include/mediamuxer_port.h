...
/*
 * @file mediamuxer_port.h
 * @brief decleartions related to port split for media muxer
 ...
 * @pre It will be invoked when the error has occured if you register this callback using mediamuxer_set_error_cb().
...
typedef struct _media_port_muxer_ops {
	...
	int (*get_data_count)(MMHandleType pHandle, int *count);
	int (*get_track_info)(MMHandleType pHandle, void *info);
